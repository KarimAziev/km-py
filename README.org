...
The same setup makes =process-environment= and =exec-path= buffer-local before
calling =eglot-ensure=.  Consequently Eglot can find a language server installed
inside =.venv= and Pyright inherits =VIRTUAL_ENV=, =PATH=, and =PYTHONPATH=
without switching the environment used by unrelated Emacs buffers.
